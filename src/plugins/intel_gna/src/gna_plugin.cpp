...
void GNAPlugin::LoadNetwork(const CNNNetwork& _network) {
    ...
    const auto effectiveCompileTarget = config.target.get_effective_compile_target(gnadevice != nullptr);
    ...
    if (_network.getFunction()) {
        ...
        manager.register_pass<ov::intel_gna::pass::Decompose2DConvTransposedWithBiasAF>(effectiveCompileTarget, config.gnaPrecision);
        manager.register_pass<ov::intel_gna::pass::Decompose2DConvTransposedWithBias>(effectiveCompileTarget, config.gnaPrecision);
...
void GNAPlugin::DumpXNNToFile() const {
    ...
    if (config.target.user_set_compile_target == common::DeviceVersionEmbedded1_0) {
