...
GNADeviceHelper::GNADeviceHelper(const Target& targetIn,
                                 bool swExactModeIn,
...
uint32_t GNADeviceHelper::enqueueRequest(const uint32_t requestConfigID, Gna2AccelerationMode gna2AccelerationMode) {
    ...
    if ((gna2AccelerationMode == Gna2AccelerationModeHardware ||
        ...
        target.detected_device_version == DeviceVersionSoftwareEmulation) {
...
void GNADeviceHelper::dumpTLVForDeviceVersion(const uint32_t modelId,
    ...
    ExportTlvModel(modelId,
                   nGnaDeviceIndex,
                   outStream,
                   target.user_set_compile_target,
                   inputsContainer,
                   outputsContainer,
                   allAllocations);
}

void GNADeviceHelper::createVirtualDevice(DeviceVersion devVersion) {
    const auto status = Gna2DeviceCreateForExport(static_cast<Gna2DeviceVersion>(devVersion), &nGnaDeviceIndex);
...
void GNADeviceHelper::open() {
    ...
    if (useDeviceEmbeddedExport) {
        createVirtualDevice(target.user_set_compile_target);
        updateGnaDeviceVersion();
    } else if (target.user_set_execution_target != DeviceVersionNotSet &&
               target.user_set_execution_target != target.detected_device_version) {
