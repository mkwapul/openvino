...
static std::shared_ptr<ngraph::Node> DecomposeDWSC(std::shared_ptr<ngraph::opset7::GroupConvolution> dwsc,
    ...
    if (bias_const) {
        ...
        reshaped_bias = ngraph::op::util::make_try_fold<ngraph::opset7::Reshape>(
            bias_const,
    ...
    // bias into account
    for (int32_t input_position = -(int32_t)pads_begin, o = 0; o < (int32_t)output_width;
         input_position += (int32_t)stride_width, o++) {
        std::shared_ptr<ngraph::Node> previous_layer_output, last_layer_output;
        int32_t filter_end = input_position + (int32_t)filter_width * (int32_t)dilation_width;
        bool first = true;

        filter_end = filter_end < input_width ? (int32_t)filter_end : (int32_t)input_width;

        for (int32_t filter_pos = input_position, filter_idx = 0; filter_pos < filter_end;
             filter_pos += (int32_t)dilation_width, filter_idx++) {
            if (filter_pos >= 0) {
...
static bool Convert(std::shared_ptr<ngraph::Node> leading_transpose,
    ...
    auto transposed_filters_const = ngraph::op::util::make_try_fold<ngraph::opset7::Transpose>(
        filters_const,
        ngraph::opset7::Constant::create(ngraph::element::i64, ngraph::Shape{5}, ngraph::Shape{4, 1, 2, 3, 0}));

    auto flat_filters_plane = ngraph::op::util::make_try_fold<ngraph::opset7::Reshape>(
...
ConvertDWSCToScaleShifts::ConvertDWSCToScaleShifts() {
    const std::string matcher_name("ConvertDWSCToScaleShifts");
